...
export interface WfcEvents {
  onGridSolved: (result: GridResult) => void;
  onAllSolved: (results: readonly GridResult[]) => void;
  onProgress: (gridIndex: number, phase: string) => void;
  onError: (error: { message: string; gridIndex?: number }) => void;
}
...
`tile-pool.ts` は glTF からタイルメッシュをロードし、タイルID をキーとするプールを構築する。ロード後のメッシュはテンプレートとして保持し、`grid-mesh.ts` でのマージ用に複製・変換して渡す。

`grid-mesh.ts` は `GridResult` を受け取り、各セルについてタイルプールからメッシュを複製、回転・高度に基づく変換を適用し、`Mesh.MergeMeshes()` で 1 グリッドあたり1メッシュに統合する。全グリッドで約19メッシュ（= 19ドローコール）。装飾物は別メッシュのため合計約38ドローコール。マテリアルは全セルで単一のパレットテクスチャ共有マテリアルを使用する。
...
export async function boot(
  ...
  ui.subscribe({
    onBuildAllRequested: async (seed) => {
      wfc.updateConfig({ seed });
      const grids = await wfc.solveAll();
      const placements = await wfc.generatePlacements(grids);
      renderer.clear();
      for (const grid of grids) renderer.addGrid(grid);
  ...
  // WFC → Render（段階的構築）
  wfc.subscribe({
    onGridSolved: (result) => renderer.addGrid(result),
  });
}
```
...
| メッシュ統合 (BatchedMesh相当) | @hex/render    | grid-mesh.ts      | `Mesh.MergeMeshes()`            |
...
Three.js の `BatchedMesh` は描画時に異なるジオメトリを1ドローコールで描画する動的バッチング機構であり、個々のインスタンスの変換を後から変更できる。Babylon.js の `Mesh.MergeMeshes()` は静的なジオメトリ結合であり、結合後の個別操作はできない。本プロジェクトではマップ生成後にタイルを個別操作する必要がないため、`MergeMeshes` で同等の結果が得られる。頂点数が 65,536 を超える場合は `allow32BitsIndices: true` を指定する。
